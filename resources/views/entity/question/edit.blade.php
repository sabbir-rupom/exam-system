...
@slot('subtitle') Edit Question @endslot
@endcomponent
...
<div class="card">
    <div class="card-body">
        ...
        <form action="{{ route('admin.question.edit', $ques) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="mb-3 row">
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-check form-radio-primary form-check-inline mt-2">
                        <input class="form-check-input show-hidden-block" data-target=".q-option-block.q-{{ $k }}"
                            data-target_sibling=".q-option-block" type="radio" name="question_type" value="{{ $k }}"
                ...
                <div class="col-md-2">
                    <select class="form-select" name="difficulty" id="select--difficulty" aria-label="Difficulty Level">
                        ...
                        <option value="{{ $k }}" {{ $ques->difficulty == $k ? 'selected' : '' }}>{{ $v }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mb-3 row">
                ...
                <div class="col-md-2">
                    <select class="form-select" id="select--group" name="group" aria-label="Course select">
                        <option {{ empty($ques->group_tag_id) ? 'selected' : '' }} value="">--</option>
                        @foreach ($groups as $item)
                        <option value="{{ $item->id }}" {{ $ques->group_tag_id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="mb-3 row"">
                ...
                <div class="col-md-10">
                    <input type="text" class="form-control" name="name" value="{{ $ques->name }}" placeholder="Enter question title">
            ...
            <div class="mb-3 block-container mt-4">
                <fieldset class="scheduler-border p-3 mb-3">
                    ...
                    <div class="q-option-block q-1 q-2 {{
                        question_type_match('single', $ques->question_type) || question_type_match('multiple', $ques->question_type)  ? '' : 'd-none' }}">
                        @php $counter = 1; @endphp
                        @if (question_type_match('single', $ques->question_type)
                        || question_type_match('multiple', $ques->question_type))
                        ...
                        <div class="row mb-3" data-index="{{ $counter }}">
                            ...
                            <div class="col-sm-7">
                                <input type="text" class="form-control" required placeholder="enter option detail"
                                    name="option[id_{{ $item['id'] }}]" value="{{ $item['content'] }}">
                            </div>
                            <div class="col-sm-2">
                                <div class="form-check form-check-right mt-2">
                                    ...
                                    <input type="hidden" class="depend-input-array"
                                        name="answer[id_{{ $item['id'] }}]" value="{{ $item['answer'] ? 1 : '' }}">
                            ...
                            <div class="col-sm-1">
                                <button type="button" class="btn btn-danger btn-sm delete-confirm"
                                    data-url="{{ url('/ajax/question/option/' . $item['id']) }}" data-parent=".row"
                    ...
                    <div class="block-action row">
                        <div class="col-md-10 offset-md-2">
                            <button type="button" class="btn btn-success btn-add-more" data-target=".option-block">
                                <i class="fas fa-plus"></i>
                            </button>
                            <button type="button" class="btn btn-danger btn-remove-more" data-target=".option-block"
                                data-min="2">
